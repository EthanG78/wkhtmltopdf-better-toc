// -*- mode: c++; tab-width: 4; indent-tabs-mode: t; eval: (progn (c-set-style
// "stroustrup") (c-set-offset 'innamespace 0)); -*- vi:set ts=4 sts=4 sw=4
// noet :
//
...
OutlineItem::OutlineItem ()
    : parent (NULL), page (-1), display (true), forwardLinks (false),
      backLinks (false)
{
}
...
void
OutlineItem::fillAnchors (const OutlineItem *other, int &anchorCounter,
                          QVector<QPair<QWebElement, QString> > &local,
                          QHash<QString, QWebElement> &anchors)
{
  if (!other || other->children.size () != children.size ()
      || other->document != document || other->value != value
  ...
  if (other)
  ...
  else
    {
      ...
      tocAnchor
          = QString ("__WKANCHOR_") + QString::number (anchorCounter++, 36);
    }
  ...
  for (int i = 0; i < children.size (); ++i)
    children[i]->fillAnchors (other ? other->children[i] : 0, anchorCounter,
                              local, anchors);
}

bool
OutlineItem::differentFrom (const OutlineItem *other) const
{
  if (other->children.size () != children.size () || other->page != page
      || other->document != document || other->value != value
      || other->display != display)
    return true;
...
void
OutlinePrivate::fillChildAnchors (OutlineItem *item,
                                  QHash<QString, QWebElement> &anchors)
{
...
void
OutlinePrivate::outlineChildren (OutlineItem *item, QPrinter *printer,
                                 int level)
{
...
void
OutlinePrivate::dumpChildren (QTextStream &stream,
  ...
  foreach (OutlineItem *item, items)
    {
      ...
      stream << "<item title=\"" << escape (item->value) << "\" page=\""
             << (item->page + prefixSum[item->document] + settings.pageOffset)
             << "\" link=\"" << escape (item->anchor) << "\" backLink=\""
             << escape (item->tocAnchor) << "\" data-toc-item=\""
             << (item->element.attribute ("data-toc-item", ""))
             << "\" data-toc-level=\""
             << (item->element.attribute ("data-toc-level", ""))
             << "\" data-toc-title=\""
             << (item->element.attribute ("data-toc-title", "")) << "\"";
...
void
Outline::dump (QTextStream &stream) const
{
  d->buildPrefixSum ();
...
*/
Outline::Outline (const settings::PdfGlobal &settings)
    : d (new OutlinePrivate (settings))
{
}
...
*/
bool
Outline::replaceWebPage (int document, const QString &name, QWebPrinter &wp,
                         QWebFrame *frame, const settings::PdfObject &ps,
                         QVector<QPair<QWebElement, QString> > &local,
                         QHash<QString, QWebElement> &anchors)
{
  QMap<QPair<int, QPair<qreal, qreal> >, QWebElement> headings;
  foreach (const QWebElement &e,
           frame->findAllElements ("h1,h2,h3,h4,h5,h6,h7,h8,h9"))
    {
      QPair<int, QRectF> location = wp.elementLocation (e);
      headings[qMakePair (location.first, qMakePair (location.second.y (),
                                                     location.second.x ()))]
          = e;
    }
  ...
  for (QMap<QPair<int, QPair<qreal, qreal> >, QWebElement>::iterator i
       = headings.begin ();
       i != headings.end (); ++i)
    {
  ...
  root->fillAnchors (d->documentOutlines[document], d->anchorCounter, local,
                     anchors);
...
*/
void
Outline::addWebPage (const QString &name, QWebPrinter &wp, QWebFrame *frame,
                     ...
                     QHash<QString, QWebElement> &anchors)
{
  Q_UNUSED (name);
  addEmptyWebPage ();
  replaceWebPage (d->documentOutlines.size () - 1, name, wp, frame, ps, local,
                  anchors);
}
...
void
OutlinePrivate::buildHFCache (OutlineItem *i, int level)
{
  buildPrefixSum ();
  if (level >= hfCache.size ())
    return;
 ...
 */
void
Outline::fillHeaderFooterParms (int page, QHash<QString, QString> &parms,
                                const settings::PdfObject &ps)
{
  // Build hfcache
  if (d->hfCache.size () == 0)
  ...
  parms["section"]
      = d->hfCache[0][page] ? d->hfCache[0][page]->value : QString ("");
  parms["subsection"]
      = d->hfCache[1][page] ? d->hfCache[1][page]->value : QString ("");
  parms["subsubsection"]
      = d->hfCache[2][page] ? d->hfCache[2][page]->value : QString ("");
}
...
*/
void
Outline::fillAnchors (int doc, QHash<QString, QWebElement> &anchors)
{
  if (doc < 0 || doc >= d->documentOutlines.size ())
    return;
...
*/
void
Outline::printOutline (QPrinter *printer)
{
  if (!d->settings.outline)
    return;
  foreach (OutlineItem *i, d->documentOutlines)
